...
4. Run the flow endpoint with `uvicorn server:app --reload` and the webhook handler with `uvicorn main:app --reload`.
5. Edit `src/flow.py` with your logic to navigate between the Flow screens.


To run server.py:  uvicorn src.server:app --reload

To run wehbook: uvicorn main:app --reload

for key generation: python -m src.key_generator your-passphrase